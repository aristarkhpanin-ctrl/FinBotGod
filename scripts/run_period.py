...
def main() -> int:
    ...
    lines = [
        ...
        "",
        "**ПРЕДУПРЕЖДЕНИЕ О ТОЧКЕ СТАРТА.** Дата начала инвестирования "
        f"({args.test_start}) выбрана задним числом, а это само по себе "
        "форма подгонки: в реальном времени вы не знали бы, что ждёт рынок "
        "после этой даты. Один период — одна случайная выборка; надёжнее "
